...
func OidcSuccessHandler(
	...
	return func(logger *log.Logger, next http.Handler, w http.ResponseWriter, r *http.Request, token string) {
		if strings.HasPrefix(r.URL.Path, sso_redirector.RedirectorPath) {
			if token == "" {
				logger.Debug("No token was passed for the request", []encoder.Field{encoder.String("requestUrlPath", r.URL.Path)})
			...
			if err != nil {
				logger.Error("No token was passed for the request", []encoder.Field{encoder.Error(err)})
				w.WriteHeader(http.StatusUnprocessableEntity)
				return
			}

			logger.Debug("Adding token session cookie and redirecting to redirect url", []encoder.Field{encoder.String("requestUrlPath", r.URL.Path), encoder.String("redirectUrl", redirectUrl.String())})
		...
		logger.Debug("Request passed validation and is passed onto the next handler", []encoder.Field{encoder.String("requestUrlPath", r.URL.Path)})
		next.ServeHTTP(w, r)
		return
	}
}
